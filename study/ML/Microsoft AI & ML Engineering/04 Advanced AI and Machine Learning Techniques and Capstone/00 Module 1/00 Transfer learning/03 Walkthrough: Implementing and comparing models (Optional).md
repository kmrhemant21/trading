...
1. Initialized the logistic regression model using `LogisticRegression()`.
2. Trained the model using the `fit()` method on the training data.
3. Used the trained model to make predictions on the test set.
4. Evaluated the model's performance by calculating its accuracy.
...
1. Initialized the decision tree model using `DecisionTreeClassifier()`.
2. Trained the model on the training data.
3. Made predictions on the test set using the `predict()` method.
4. Evaluated the model by calculating its accuracy.
