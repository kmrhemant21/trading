...
try:
    ...
    # If running in an environment without these, install them locally.
    pass
...
log = logging.getLogger("ema_rsi_adx_v6_cap25")
...
class Config:
    ...
    ema_fast: int   = 10
    ema_slow: int   = 50
    ema_htf:  int   = 200       # higher timeframe trend EMA
    ...
    # ADX
    adx_len: int = 14
    adx_min: float = 25.0       # require ADX > 20

    # Fixed exits
    stop_loss_pct: float = 0.05   # 3% stop
    target_pct: float    = 0.10   # 6% take profit

    # Portfolio
    apply_fees: bool    = True
    initial_capital: float = 200_000.0
    max_concurrent_positions: int = 4
    deploy_cash_frac: float = 0.25   # NEW: cap daily deployment to 25% of available cash
    ...
    # Optional guards (OFF by default)
...
def simulate_ticker(ticker: str, df: pd.DataFrame, cfg: Config):
    ...
    cols = ["ticker","side","date","price","shares","reason","signal_reason","score",
            "rsi","adx","ema_fast","ema_slow","ema_htf","trend_strength","close","high_52w"]
    if d.empty:
    ...
    rsi_cross = (d["rsi_prev"] < cfg.rsi_buy_max) & (d["rsi"] >= cfg.rsi_buy_max)
    adx_ok  = (d["adx"] > cfg.adx_min)

    entry_signal = trend_ok & htf_ok & rsi_cross & adx_ok
    exit_ind = (d["rsi"] > cfg.rsi_sell_min) | (d["ema_fast"] < d["ema_slow"])
        ...
        if not in_pos:
            if entry_signal.loc[dt] and basic_liquidity_ok(row, cfg):
                ...
                if abs(gap) <= 0.02:
                    ...
                    sig_reason = "RSI cross ↑30; EMA20>EMA50; Close>EMA200; ADX>20"
                    trades.append({
                        ...
                        "trend_strength": float(row["trend_strength"]), "close": float(row["Close"]),
    ...
    for dt in dates:
        ...
        for _, tr in day_trades[day_trades["side"] == "SELL"].iterrows():
            ...
            completed_legs.append({
                ...
                "trend_strength": tr.get("trend_strength", np.nan), "close": tr.get("close", np.nan), "high_52w": tr.get("high_52w", np.nan),
        ...
        if not selected.empty:
            ...
            # --- NEW: Cap daily deployed cash to deploy_cash_frac of available cash ---
            ...
            if deploy_cash <= 0:
                ...
                for w_amt, t in zip(alloc, names):
                    ...
                    # NOTE: weight fraction is relative to today's *deployable* cash pool
                    mvo_weight_today = (w_amt / deploy_cash) if deploy_cash > 0 else 0.0
                    sig_reason = row_sel.get("signal_reason", "RSI cross ↑30; EMA20>EMA50; Close>EMA200; ADX>20")
                    reason_text = (
                    ...
                    completed_legs.append({
                        ...
                        "trend_strength": float(row_sel.get("trend_strength", np.nan)), "close": close_val, "high_52w": high_52w,
    ...
    for _, leg in legs_df.iterrows():
        ...
        if leg["side"] == "BUY":
            ...
            roundtrips.append({
                ...
                # carry indicators from entry snapshot + selection meta
                ...
                "trend_strength_entry": float(buy.get("trend_strength", np.nan)),
...
def backtest(cfg: Config):
    ...
    log.info("Signals: generating EMA/RSI/ADX and candidate entries...")
    all_trades = []
    ...
    if not all_trades:
        log.warning("No signals generated; check your thresholds.")
