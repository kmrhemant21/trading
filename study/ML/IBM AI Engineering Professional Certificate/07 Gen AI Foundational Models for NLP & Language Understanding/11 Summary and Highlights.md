...
- The **continuous bag of words (CBOW) model** utilizes context words to predict a target word and generate its embedding.
...
- Word embeddings utilize Stanford pretrained **GloVe** (global vectors) that leverage large-scale data for word embeddings.
...
- RNNs only remember short-term information and are challenging to train. Two popular RNN enhancements are **Gated Recurrent Units (GRUs)** and **Long Short-Term Memory (LSTMs)**.
...
1. Initialize the model in training mode to activate essential layers like dropout, ensuring optimal performance during training.

2. Iterate through training data batches, assigning input (src) and target (trg) sequences to the correct device.

3. Generate predictions by output.

4. Reshape the output tensor, which aligns the rows and columns correctly for loss calculation.

5. Calculate the average loss per batch after processing all batches.
